...
//
// For a brand new entry, lastSHA is seeded from status.observedRevision so the
// first poll after leader startup does not produce a spurious annotation patch
// (the remote SHA will match what the Workspace controller already applied).
...
func (r *RefWatcherReconciler) upsertEntry(key types.NamespacedName, url, ref string, interval time.Duration, observedRevision string) {
	...
	// New entry: seed lastSHA from status.observedRevision to avoid a
	// spurious trigger on first poll after leader startup. Apply jitter to
	// nextPollAt so that a mass re-sync (e.g. leader failover reconciling
	// hundreds of Workspaces) does not slam the Git host with concurrent
	// ls-remote calls.
	entry := &registryEntry{
		url:        url,
		ref:        ref,
...
func (r *RefWatcherReconciler) pollRemote(ctx context.Context, key types.NamespacedName) {
	...
	// This happens when a new Workspace is created with a branch ref like
	// "main" — observedRevision is blank, so upsertEntry seeds lastSHA as "".
	// The first poll resolves "main" to a real commit SHA and sees a diff, but
	// this is not a genuine change: the user just created the Workspace and
	// the Workspace controller should run its initial plan/apply using the
	// branch name from the spec, not a SHA injected by the RefWatcher. We
	// silently record the resolved SHA so that subsequent polls have a
	// baseline to compare against, but we do not patch the annotation. The
	// next time the branch moves to a different commit the RefWatcher will
	// detect the real change and annotate the Workspace normally.
	if lastSHA == "" {
