...
| `auth.baseURL`                   | Base URL of the Magos UI as seen by the browser (e.g. `https://magos.example.com`). Used to build the OIDC redirect URI. Defaults to the ingress/TLS URL when ingress is enabled, or `http://localhost` otherwise.                                                                                                                                     | `""`               |
...
| `auth.oidc.clientSecretRef.name` | Name of an existing Secret containing the OIDC client secret. Required for providers that use the confidential client type (e.g. Google "Web application"). Leave empty for public clients (e.g. Google "Desktop app" type, or providers that support PKCE without a secret).                                                                          | `""`               |
| `auth.oidc.clientSecretRef.key`  | Key within the Secret that holds the client secret value.                                                                                                                                                                                                                                                                                              | `oidcClientSecret` |
